...
def generate_syn_feature(netG, netE, classes, attribute, device, opt, syn_num):
    ...
    with torch.no_grad():
        for i in range(nclass):
            ...
            syn_feature.narrow(0, i * syn_num, syn_num).copy_(emb_output.detach().cpu())
...
def generate_syn_aug_feature_after(netG, netE, classes, attribute, device, opt, syn_num, dataset):
    ...
    with torch.no_grad():
        embed_feature = netE(dataset.train_feature.to(device), emb=True)  # real embed
        ...
        for i in range(nclass):
            ...
            closer_seen_center = tr_cls_centroid[dataset.s_unseen_idx_mat[i]]
            calib_fea = torch.einsum('i,ik->k', [weight, closer_seen_center.detach().cpu()])
            iclass = classes[i]
            iclass_att = attribute[iclass]
            syn_att.copy_(iclass_att.repeat(syn_num, 1))
            syn_noise.normal_(0, 1)
            ...
            calib_fea_dev = calib_fea.to(device)
            mix_fea = opt.gamma * emb_output + (1 - opt.gamma) * calib_fea_dev.unsqueeze(0)

            syn_feature.narrow(0, i * syn_num, syn_num).copy_(mix_fea.detach().cpu())
            syn_feature_no_mix.narrow(0, i * syn_num, syn_num).copy_(emb_output.detach().cpu())
            syn_proj_feature_no_mix.narrow(0, i * syn_num, syn_num).copy_(outz.detach().cpu())
            syn_label.narrow(0, i * syn_num, syn_num).fill_(iclass)
