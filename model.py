...
class Embedding_model(nn.Module):
    ...
    def get_center(self):
        # Model may still be on CPU during __init__, compute on CPU then move
        center_emb = self.lrelu(self.fc1(self.dataset.tr_cls_centroid))
        center = self.fc2(center_emb).detach()
    ...
    def forward(self, features, label=None, local_label=None, emb=False, retrieval=False):
        ...
        mask = F.one_hot(local_label, num_classes=self.dataset.seenclass_num).float().to(features.device)
...
def MultiClassCrossEntropy(logits, labels):
    ...
    outputs = torch.log_softmax(logits_normed, dim=1)  # compute the log of softmax values
