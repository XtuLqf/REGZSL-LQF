...
class RkdDistance(nn.Module):
    def forward(self, student, teacher, opt):
        # class_means = []
        # for i in range(0,student.shape[0],opt.shot):
        #     class_mean = torch.mean(student[i],dim=1)
        #     class_means.append(class_mean)
        # student_mean = torch.stack(class_means)
        with torch.no_grad():
...
class Pre_RkdDistance(nn.Module):
    def forward(self, student, label, dataset, opt):
        # class_means = []
        # for i in range(0,student.shape[0],opt.shot):
        #     class_mean = torch.mean(student[i],dim=1)
        #     class_means.append(class_mean)
        # student_mean = torch.stack(class_means)
        t_d = torch.FloatTensor(label.shape[0], label.shape[0]).cuda()
        with torch.no_grad():
            for i in range(label.shape[0]):
              t_d[i]= dataset.att_cos_matrix[label[i]][label]
        d = student.mm(student.t())
        loss = F.smooth_l1_loss(d, t_d, reduction='mean')
        return loss
