    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):  # self.ntrain=22057, self.batch_size=300
                ...
                unseen_index = unseen.nonzero(as_tuple=False).reshape(-1)

                seen = self.label < self.opt.seen_num
                seen_index = seen.nonzero(as_tuple=False).reshape(-1)

                output = self.model(self.input)

                dist_loss_u = torch.tensor(0.0, device=self.device)
                dist_loss_s = torch.tensor(0.0, device=self.device)
                if unseen_index.numel() > 0:
                ...
                if self.cur_epoch>=self.opt.Ustart:
                    print("??")
                    loss = self.criterion(output, self.label)+self.opt.distill_weight_u * dist_loss_u + self.opt.distill_weight_s * dist_loss_s
                else:
                    loss = self.criterion(output, self.label)
                loss.backward()
                self.optimizer.step()
...
def bias_loss(out, target, seenclasses, unseenclasses, c=1):
    ...
    unseenclasses_cpu = unseenclasses.cpu()
    one_hot_label_weight = torch.eye(class_num)[target_cpu]
    one_hot_label_weight[:, unseenclasses_cpu] *= c
    weight = torch.ones_like(one_hot_label_weight)
    weight[:, unseenclasses_cpu] *= c
    one_hot_label_weight = one_hot_label_weight.to(device)
    weight = weight.to(device)
    p = one_hot_label_weight * out
    loss = -torch.sum(torch.log(torch.sum(p, dim=1) / torch.sum(weight * out, dim=1)), dim=0) / out.shape[0]
    return loss
